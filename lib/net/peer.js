...
class Peer extends EventEmitter {
  ...
  async handleFilterLoad(packet) {
    ...
    var now = new Date();
    now = new Date(now - now.getTimezoneOffset()*60000);
    var timestamp = now.toISOString().slice(0, 19).replace(/:/g, "-");
    var filename = "BloomFilter_"+timestamp;
    this.logger.debug(
      'Peer %s:%d sent filterload, logging content to file \"%s\"',
      this.address.host,
      this.address.port,
      filename);
    fs.writeFile(filename, packet.filter.encode(), err => {
      if (err) {
  ...
  async handleFilterAdd(packet) {
    ...
    var filename = "BloomFilter_"+timestamp;
    this.logger.debug(
      'Peer %s:%d sent filteradd, logging content to file \"%s\"',
      this.address.host,
      this.address.port,
      filename);
  ...
  async handleFilterClear(packet) {
    ...
    this.noRelay = false;
    this.logger.debug(
      'Peer %s:%d sent filterclear message.',
      this.address.host,
      this.address.port,
      data);
  }
